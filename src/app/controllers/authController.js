...
router.post('/forgot_password', async (req, res) => {
    ...
    try {
        ...
        mailer.sendMail({
            from: '"Task Board Crud Api 👻" <foo@example.com>',
            to: email,
            subject: "Hello ✔",
            ...
            if(err) { 
                console.log(err); res.status(400).send({ error: 'Cannot send forgot email password'}) 
            } else { 
...
router.post('/reset_password', async(req, res) => {
    ...
    try {
        ...
        if(token !== user.passwordResetToken)
            return res.status(400).send({ error: 'Password reset token is invalid'});

        const now = new Date();
        if(now > user.passwordResetExpires)
            return res.status(400).send({ error: 'Password reset token expired, generate a new one' });
