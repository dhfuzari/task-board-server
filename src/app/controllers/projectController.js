  ...
  try {
    ...
    if (Array.isArray(tasks)) {
      await Promise.all(tasks.map(async (task) => {
        // "new Task()" is an alternative syntax to Task.create().
        // It creates a new Task but it doesn't save it.
...
router.put('/:projectId', async (req, res) => {
  try {
    ...
    // "{ new: true }" config param returns the new object updated by fundByIdAndUpdate function
